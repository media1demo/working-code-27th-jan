...
//tion to apply blugepinch effect to selected regions
function blugepinchSelectedRegions(imageData, selectedRegions, maxVerticalOffset) {
    const width = imageData.width;
    const height = imageData.height;
    ...
    selectedRegions.forEach(region => {
        const verticalOffset = Math.random() * maxVerticalOffset;          // Down movement
        const horizontalOffset = -(Math.random() * maxVerticalOffset);     // Left movement
        ...
        // Apply blugepinch effect to selected region
        region.forEach(pixelIndex => {
            const x = pixelIndex % width;
            const y = Math.floor(pixelIndex / width);
            const newY = Math.min(height - 1, y + verticalOffset * (1 - (x / width)));
            const newX = Math.max(0, x + horizontalOffset * (1 - (y / height)));
...
self.onmessage = function(e) {
    ...
    try {
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            ...
            resultImageData = blugepinchSelectedRegions(imageData, selectedRegions, maxVerticalOffset);
            ...
            blugepinch(resultImageData, totalMove);
            currentIteration++;
            progress = undefined;
        }
