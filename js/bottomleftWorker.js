...
function moveRegionBottomLeft(imageData, totalMove) {
    ...
    // Move pixels to new position (bottom-left)
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const newY = Math.min(y + totalMove, height - 1);  // Move down
            const newX = Math.max(x - totalMove, 0);          // Move left
...
// Function to move selected regions bottom-left
function moveSelectedRegionsBottomLeft(imageData, selectedRegions, maxVerticalOffset) {
    const width = imageData.width;
    const height = imageData.height;
...
self.onmessage = function(e) {
    ...
    try {
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            ...
            resultImageData = moveSelectedRegionsBottomLeft(imageData, selectedRegions, maxVerticalOffset);
            ...
            moveRegionBottomLeft(resultImageData, totalMove);
            currentIteration++;
            progress = undefined;
        }
