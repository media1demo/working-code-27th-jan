...
const DEFAULT_MAX_HORIZONTAL_OFFSET = 50;  // Added horizontal offset
...
function moveRegionDownLeft(imageData, totalMoveVertical, totalMoveHorizontal) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            const newX = Math.max(0, x - totalMoveHorizontal);  // Move left
...
function moveSelectedRegionsDownLeft(imageData, selectedRegions, maxVerticalOffset, maxHorizontalOffset) {
    const width = imageData.width;
    const height = imageData.height;
    ...
    selectedRegions.forEach(region => {
        ...
        region.forEach(pixelIndex => {
            ...
            const newX = Math.max(0, x - horizontalOffset);  // Move left
...
self.onmessage = function(e) {
    const { 
        ...
        value3: maxHorizontalOffset = DEFAULT_MAX_HORIZONTAL_OFFSET,  // Added horizontal offset parameter
    ...
    try {
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            ...
            resultImageData = moveSelectedRegionsDownLeft(imageData, selectedRegions, maxVerticalOffset, maxHorizontalOffset);
            ...
            moveRegionDownLeft(resultImageData, totalMoveVertical, totalMoveHorizontal);
            currentIteration++;
            progress = undefined;
        }
