...
const DEFAULT_MAX_HORIZONTAL_OFFSET = 50;  // Horizontal offset
...
function moveRegionDownRight(imageData, totalMoveVertical, totalMoveHorizontal) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const newY = Math.min(y + totalMoveVertical, height - 1);
            const newX = Math.min(x + totalMoveHorizontal, width - 1);  // Move right
...
function moveSelectedRegionsDownRight(imageData, selectedRegions, maxVerticalOffset, maxHorizontalOffset) {
    const width = imageData.width;
    const height = imageData.height;
    ...
    selectedRegions.forEach(region => {
        ...
        const horizontalOffset = Math.random() * maxHorizontalOffset;  // Random horizontal offset
        ...
        region.forEach(pixelIndex => {
            ...
            const newX = Math.min(width - 1, x + horizontalOffset);  // Move right
...
self.onmessage = function(e) {
    const { 
        ...
        value3: maxHorizontalOffset = DEFAULT_MAX_HORIZONTAL_OFFSET,  // Horizontal offset parameter
    ...
    try {
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            ...
            resultImageData = moveSelectedRegionsDownRight(imageData, selectedRegions, maxVerticalOffset, maxHorizontalOffset);
            ...
            const totalMoveVertical = DEFAULT_MOVE_STEP * (currentIteration + 1);
            const totalMoveHorizontal = DEFAULT_MOVE_STEP * (currentIteration + 1);
            moveRegionDownRight(resultImageData, totalMoveVertical, totalMoveHorizontal);
            currentIteration++;
            progress = undefined;
        }
