const DEFAULT_CYCLE_LENGTH = 8; // Number of frames in the jump cycle
const DEFAULT_JUMP_AMPLITUDE = 0.2; // Amplitude of the horizontal jump effect
const DEFAULT_VERTICAL_BOB_AMPLITUDE = 0.1; // Amplitude of the vertical bobbing effect
...
function applyJumpLeftEffect(imageData, selectedRegions, t) {
    const { width, height } = imageData;
    const newImageData = createTransparentImageData(width, height);

    // Animation parameters
    const cycleLength = DEFAULT_CYCLE_LENGTH;
    const phase = (t * cycleLength) % 1; // Current phase of the jump cycle
    const jumpAmplitude = DEFAULT_JUMP_AMPLITUDE; // Amplitude of the jump effect
    const horizontalJump = jumpAmplitude * Math.sin(phase * Math.PI * 2); // Horizontal jump movement
    const verticalBob = DEFAULT_VERTICAL_BOB_AMPLITUDE * Math.sin(phase * Math.PI * 2); // Bobbing up and down
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for jump and bobbing
        const padding = Math.ceil(Math.max(width, height) * (jumpAmplitude + DEFAULT_VERTICAL_BOB_AMPLITUDE));
        ...
        // Process expanded area for jump and bobbing effects
        for (let y = expandedMinY; y <= expandedMaxY; y++) {
            for (let x = expandedMinX; x <= expandedMaxX; x++) {
                ...
                // Apply horizontal jump effect (moving left)
                nx -= horizontalJump;

                // Apply vertical bobbing effect
                ny += verticalBob;
...
self.onmessage = function(e) {
    try {
        const {
            ...
            jumpAmplitude = DEFAULT_JUMP_AMPLITUDE,
            verticalBobAmplitude = DEFAULT_VERTICAL_BOB_AMPLITUDE
        } = e.data;
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            resultImageData = applyJumpLeftEffect(imageData, selectedRegions, value);
