// Placeholder for leftbottomzerorighttopWorker.js
...
//     const cycleLength = 5; // Number of frames in the sprint cycle
//     const phase = (t * cycleLength) % 1; // Current phase of the sprint cycle
//     const forwardSprint = 0.15 * Math.sin(phase * Math.PI * 2); // Forward sprint effect
...
const DEFAULT_CYCLE_LENGTH = 5; // Number of frames in the sprint cycle
const DEFAULT_ZOOM_AMPLITUDE = 0.2; // Amplitude of the zoom effect
const DEFAULT_FORWARD_SPRINT_AMPLITUDE = 0.15; // Amplitude of the forward sprint effect
...
function applyZoomEffect(imageData, selectedRegions, t) {
    ...
    const forwardSprint = DEFAULT_FORWARD_SPRINT_AMPLITUDE * Math.sin(phase * Math.PI * 2); // Forward sprint effect
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and sprint effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + Math.abs(DEFAULT_FORWARD_SPRINT_AMPLITUDE)));
        ...
        // Process expanded area for zoom and sprint effects
...
self.onmessage = function(e) {
    try {
        const {
            ...
            forwardSprintAmplitude = DEFAULT_FORWARD_SPRINT_AMPLITUDE
        } = e.data;
