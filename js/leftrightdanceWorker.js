// Placeholder for leftrightdanceWorker.js
...
//     const cycleLength = 4; // Number of frames in the oscillation cycle
//     const phase = (t * cycleLength) % 1; // Current phase of the oscillation cycle
//     const oscillationAmplitude = 0.1; // Amplitude of the oscillation effect
...
//             nx += oscillationAmplitude * Math.sin(ny * Math.PI * 2 + phase * Math.PI * 2);
...
// };
const DEFAULT_CYCLE_LENGTH = 4; // Number of frames in the oscillation cycle
const DEFAULT_OSCILLATION_AMPLITUDE = 0.1; // Amplitude of the oscillation effect
...
function applyOscillationEffect(imageData, selectedRegions, t) {
    ...
    const phase = (t * cycleLength) % 1; // Current phase of the oscillation cycle
    const oscillationAmplitude = DEFAULT_OSCILLATION_AMPLITUDE; // Amplitude of the oscillation effect
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for oscillation
        const padding = Math.ceil(width * oscillationAmplitude);
        ...
        // Process expanded area for oscillation effect
...
self.onmessage = function(e) {
    try {
        const {
            ...
            oscillationAmplitude = DEFAULT_OSCILLATION_AMPLITUDE
        } = e.data;
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            resultImageData = applyOscillationEffect(imageData, selectedRegions, value);
