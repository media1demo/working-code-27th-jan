...
//     // Function to clone and move the selected region to the opposite side
...
//             // Apply horizontal shift to the opposite side
...
// Function to move entire region left
function moveRegionLeft(imageData, totalMove) {
    const width = imageData.width;
    const height = imageData.height;
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const newX = Math.max(x - totalMove, 0);
            if (newX !== x) {
...
function moveSelectedRegions(imageData, selectedRegions, maxHorizontalOffset) {
    const width = imageData.width;
    const height = imageData.height;
...
self.onmessage = function(e) {
    ...
    try {
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            ...
            resultImageData = moveSelectedRegions(imageData, selectedRegions, maxHorizontalOffset);
            ...
            moveRegionLeft(resultImageData, totalMove);
            currentIteration++;
            progress = undefined;
        }
