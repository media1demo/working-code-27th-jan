...
function processCombinedEffects(imageData, selectedRegions, params) {
    ...
    if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
        ...
        // Scale rotation based on region size
        const regionSize = Math.max(bounds.width, bounds.height);
        const rotationScale = Math.min(1, regionSize / 100); // Scale rotation for smaller regions
        
        // Create 3D rotation matrix for this region
        const rotation = create3DYRotationMatrix(
            Math.sin(currentIteration * params.rotationSpeed) * 0.5 * rotationScale, // Oscillating and bounded rotation
            bounds.centerX,
            bounds.centerY,
            params.perspective * 0.5 // Reduced perspective effect
        );
    ...
    // Apply VR effects
    processedImage = applyVREffects(processedImage, currentIteration);

    // Apply curve distortion to selected regions
    const curveIntensity = params.curveIntensity || 0.5; // Default curve intensity
    const distortedImage = applyCurveDistortion(processedImage, curveIntensity);

    // Blend the distorted regions back into the processed image
    if (selectedRegions?.length > 0) {
        selectedRegions.flat().forEach(pixelIndex => {
            const idx = pixelIndex * 4;
            for (let c = 0; c < 4; c++) {
                processedImage.data[idx + c] = distortedImage.data[idx + c];
            }
        });
    }
