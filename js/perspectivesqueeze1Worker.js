...
//     const result = applySqueezeAroundPoints(imageData, value, clickedPoints);
//     self.postMessage({ imageData: result });
// };

// function applySqueezeAroundPoints(imageData, value, clickedPoints) {
...
//                 const squeeze = Math.exp(-distance * 0.005);
...
//             if (maxSqueeze > 0) {
//                 const squeezeX = 1 + value.squeezeX * maxSqueeze;
//                 const squeezeY = 1 + value.squeezeY * maxSqueeze;

//                 const sourceX = Math.round(closestPoint.x + (x - closestPoint.x) / squeezeX);
//                 const sourceY = Math.round(closestPoint.y + (y - closestPoint.y) / squeezeY);
