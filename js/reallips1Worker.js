function applyStretchEffect(imageData, selectedRegions, strength, radius) {
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                if (distance < effectRadius) {
                    // Calculate vertical displacement with smooth falloff
                    const factor = 1 - (distance / effectRadius);
                    const displacement = Math.sign(dy) * factor * factor * effectStrength * 30;
...
self.onmessage = function(e) {
    ...
    try {
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            resultImageData = applyStretchEffect(imageData, selectedRegions, strength, radius);
        } else {
            // If no regions selected, apply to entire image
            resultImageData = applyStretchEffect(imageData, 
