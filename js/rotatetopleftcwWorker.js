...
function rotateRegionUpperLeft(imageData, angle) {
    ...
    const centerX = 0;  // Rotate around upper left
    const centerY = 0;  // Key difference from bottom left version
    
    // Rotate pixels around upper left corner
...
// Function to rotate selected regions around upper left corner
function rotateSelectedRegionsUpperLeft(imageData, selectedRegions, maxRotationAngle) {
    const width = imageData.width;
    const height = imageData.height;
...
self.onmessage = function(e) {
    ...
    try {
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            ...
            resultImageData = rotateSelectedRegionsUpperLeft(imageData, selectedRegions, maxRotationAngle);
            ...
            rotateRegionUpperLeft(resultImageData, totalRotation);
            currentIteration++;
            progress = undefined;
        }
