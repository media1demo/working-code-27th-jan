...
//     const cycleLength = 6; // Number of frames in the jump cycle
//     const phase = (t * cycleLength) % 1; // Current phase of the jump cycle
//     const jumpHeight = 0.2 * Math.sin(phase * Math.PI * 2); // Height of the jump
...
const DEFAULT_CYCLE_LENGTH = 6; // Number of frames in the jump cycle
const DEFAULT_ZOOM_AMPLITUDE = 0.2; // Amplitude of the zoom effect
const DEFAULT_JUMP_HEIGHT_AMPLITUDE = 0.2; // Amplitude of the jump effect
...
function applyZoomEffect(imageData, selectedRegions, t) {
    ...
    const jumpHeight = DEFAULT_JUMP_HEIGHT_AMPLITUDE * Math.sin(phase * Math.PI * 2); // Height of the jump
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and jump effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + DEFAULT_JUMP_HEIGHT_AMPLITUDE));
        ...
        // Process expanded area for zoom and jump effects
...
self.onmessage = function(e) {
    try {
        const {
            ...
            jumpHeightAmplitude = DEFAULT_JUMP_HEIGHT_AMPLITUDE
        } = e.data;
