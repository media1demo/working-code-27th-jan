...
//     const backwardShift = -0.1 * Math.sin(phase * Math.PI * 2); // Backward shift effect
...
const DEFAULT_BACKWARD_SHIFT_AMPLITUDE = 0.1; // Amplitude of the backward shift effect
...
function applyZoomEffect(imageData, selectedRegions, t) {
    ...
    const backwardShift = DEFAULT_BACKWARD_SHIFT_AMPLITUDE * Math.sin(phase * Math.PI * 2); // Backward shift effect
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and backward shift effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + Math.abs(DEFAULT_BACKWARD_SHIFT_AMPLITUDE)));
        ...
        // Process expanded area for zoom and backward shift effects
...
self.onmessage = function(e) {
    try {
        const {
            ...
            backwardShiftAmplitude = DEFAULT_BACKWARD_SHIFT_AMPLITUDE
        } = e.data;
