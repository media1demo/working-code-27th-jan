...
//     const cycleLength = 12; // Number of frames in the rotation cycle
//     const phase = (t * cycleLength) % 1; // Current phase of the rotation cycle
//     const rotationalSpeed = 0.1; // Speed of the rotational motion
...
//             let angle = rotationalSpeed * phase * Math.PI * 2;
//             let cosA = Math.cos(angle);
//             let sinA = Math.sin(angle);
//             let tx = cosA * nx - sinA * ny;
//             let ty = sinA * nx + cosA * ny;
//             nx = tx;
//             ny = ty;
...
const DEFAULT_CYCLE_LENGTH = 12; // Number of frames in the rotation cycle
const DEFAULT_ZOOM_AMPLITUDE = 0.2; // Amplitude of the zoom effect
const DEFAULT_ROTATIONAL_SPEED = 0.1; // Speed of the rotational motion
...
function applyZoomEffect(imageData, selectedRegions, t) {
    ...
    const rotationalSpeed = DEFAULT_ROTATIONAL_SPEED; // Speed of the rotational motion
    const angle = rotationalSpeed * phase * Math.PI * 2; // Current rotation angle
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and rotation effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + 1)); // Account for zoom and rotation
        ...
        // Process expanded area for zoom and rotation effects
        for (let y = expandedMinY; y <= expandedMaxY; y++) {
            for (let x = expandedMinX; x <= expandedMaxX; x++) {
                ...
                // Apply rotational motion
                const tx = cosA * nx - sinA * ny;
                const ty = sinA * nx + cosA * ny;
                nx = tx;
                ny = ty;
...
self.onmessage = function(e) {
    try {
        const {
            ...
            rotationalSpeed = DEFAULT_ROTATIONAL_SPEED
        } = e.data;
