...
const DEFAULT_CYCLE_LENGTH = 7; // Number of frames in the lunge cycle
const DEFAULT_ZOOM_AMPLITUDE = 0.2; // Amplitude of the zoom effect
const DEFAULT_FORWARD_LUNGE_AMPLITUDE = 0.1; // Amplitude of the forward lunge effect
const DEFAULT_DOWNWARD_LUNGE_AMPLITUDE = 0.1; // Amplitude of the downward lunge effect
...
function applyZoomEffect(imageData, selectedRegions, t) {
    ...
    const phase = (t * cycleLength) % 1; // Current phase of the lunge cycle
    ...
    const forwardLunge = DEFAULT_FORWARD_LUNGE_AMPLITUDE * Math.sin(phase * Math.PI * 2); // Forward lunge effect
    const downwardLunge = DEFAULT_DOWNWARD_LUNGE_AMPLITUDE * Math.cos(phase * Math.PI * 2); // Downward lunge effect
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and lunge effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + Math.max(DEFAULT_FORWARD_LUNGE_AMPLITUDE, DEFAULT_DOWNWARD_LUNGE_AMPLITUDE)));
        ...
        // Process expanded area for zoom and lunge effects
...
self.onmessage = function(e) {
    try {
        const {
            ...
            forwardLungeAmplitude = DEFAULT_FORWARD_LUNGE_AMPLITUDE,
            downwardLungeAmplitude = DEFAULT_DOWNWARD_LUNGE_AMPLITUDE
        } = e.data;
