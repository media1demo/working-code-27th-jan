...
//     const cycleLength = 6; // Number of frames in the skip cycle
//     const phase = (t * cycleLength) % 1; // Current phase of the skip cycle
//     const skipHeight = 0.1 * Math.sin(phase * Math.PI * 2); // Height of the skip
//     const forwardSkip = 0.05 * Math.cos(phase * Math.PI * 2); // Forward movement of the skip
...
const DEFAULT_CYCLE_LENGTH = 6; // Number of frames in the skip cycle
const DEFAULT_ZOOM_AMPLITUDE = 0.2; // Amplitude of the zoom effect
const DEFAULT_SKIP_HEIGHT_AMPLITUDE = 0.1; // Amplitude of the skip height effect
const DEFAULT_FORWARD_SKIP_AMPLITUDE = 0.05; // Amplitude of the forward skip effect
...
function applySkipEffect(imageData, selectedRegions, t) {
    ...
    const skipHeight = DEFAULT_SKIP_HEIGHT_AMPLITUDE * Math.sin(phase * Math.PI * 2); // Skip height effect
    const forwardSkip = DEFAULT_FORWARD_SKIP_AMPLITUDE * Math.cos(phase * Math.PI * 2); // Forward skip effect
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and skip effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + Math.max(DEFAULT_SKIP_HEIGHT_AMPLITUDE, DEFAULT_FORWARD_SKIP_AMPLITUDE)));
        ...
        // Process expanded area for zoom and skip effects
...
self.onmessage = function(e) {
    try {
        const {
            ...
            skipHeightAmplitude = DEFAULT_SKIP_HEIGHT_AMPLITUDE,
            forwardSkipAmplitude = DEFAULT_FORWARD_SKIP_AMPLITUDE
        } = e.data;
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            resultImageData = applySkipEffect(imageData, selectedRegions, value);
