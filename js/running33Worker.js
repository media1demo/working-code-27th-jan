...
const DEFAULT_CYCLE_LENGTH = 6; // Number of frames in the swing cycle
const DEFAULT_ZOOM_AMPLITUDE = 0.2; // Amplitude of the zoom effect
const DEFAULT_SWING_ANGLE_AMPLITUDE = 0.1; // Amplitude of the swing angle effect
...
function applySwingEffect(imageData, selectedRegions, t) {
    ...
    const swingAngle = DEFAULT_SWING_ANGLE_AMPLITUDE * Math.sin(phase * Math.PI * 2); // Swing angle effect
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and swing effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + DEFAULT_SWING_ANGLE_AMPLITUDE));
        ...
        // Process expanded area for zoom and swing effects
...
self.onmessage = function(e) {
    try {
        const {
            ...
            swingAngleAmplitude = DEFAULT_SWING_ANGLE_AMPLITUDE
        } = e.data;
