...
//     const cycleLength = 4; // Number of frames in the dodge cycle
//     const phase = (t * cycleLength) % 1; // Current phase of the dodge cycle
//     const dodgeShift = 0.15 * Math.sin(phase * Math.PI * 2); // Dodge shift effect
...
const DEFAULT_CYCLE_LENGTH = 4; // Number of frames in the dodge cycle
const DEFAULT_ZOOM_AMPLITUDE = 0.2; // Amplitude of the zoom effect
const DEFAULT_DODGE_SHIFT_AMPLITUDE = 0.15; // Amplitude of the dodge shift effect
...
function applyDodgeEffect(imageData, selectedRegions, t) {
    ...
    const dodgeShift = DEFAULT_DODGE_SHIFT_AMPLITUDE * Math.sin(phase * Math.PI * 2); // Dodge shift effect
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and dodge effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + DEFAULT_DODGE_SHIFT_AMPLITUDE));
        ...
        // Process expanded area for zoom and dodge effects
...
self.onmessage = function(e) {
    try {
        const {
            ...
            dodgeShiftAmplitude = DEFAULT_DODGE_SHIFT_AMPLITUDE
        } = e.data;
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            resultImageData = applyDodgeEffect(imageData, selectedRegions, value);
