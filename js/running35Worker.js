...
//     const cycleLength = 10; // Number of frames in the twist cycle
//     const phase = (t * cycleLength) % 1; // Current phase of the twist cycle
//     const twistAngle = 0.1 * Math.sin(phase * Math.PI * 2); // Twist angle
...
const DEFAULT_CYCLE_LENGTH = 10; // Number of frames in the twist cycle
const DEFAULT_ZOOM_AMPLITUDE = 0.2; // Amplitude of the zoom effect
const DEFAULT_TWIST_ANGLE_AMPLITUDE = 0.1; // Amplitude of the twist angle effect
...
function applyTwistEffect(imageData, selectedRegions, t) {
    ...
    const twistAngle = DEFAULT_TWIST_ANGLE_AMPLITUDE * Math.sin(phase * Math.PI * 2); // Twist angle effect
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and twist effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + DEFAULT_TWIST_ANGLE_AMPLITUDE));
        ...
        // Process expanded area for zoom and twist effects
...
self.onmessage = function(e) {
    try {
        const {
            ...
            twistAngleAmplitude = DEFAULT_TWIST_ANGLE_AMPLITUDE
        } = e.data;
