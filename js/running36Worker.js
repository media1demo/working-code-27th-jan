...
//     const cycleLength = 5; // Number of frames in the hop cycle
//     const phase = (t * cycleLength) % 1; // Current phase of the hop cycle
//     const hopHeight = 0.1 * Math.sin(phase * Math.PI * 2); // Height of the hop
//     const forwardHop = 0.05 * Math.cos(phase * Math.PI * 2); // Forward movement of the hop
...
const DEFAULT_CYCLE_LENGTH = 5; // Number of frames in the hop cycle
const DEFAULT_ZOOM_AMPLITUDE = 0.2; // Amplitude of the zoom effect
const DEFAULT_HOP_HEIGHT_AMPLITUDE = 0.1; // Amplitude of the hop height effect
const DEFAULT_FORWARD_HOP_AMPLITUDE = 0.05; // Amplitude of the forward hop effect
...
function applyHopEffect(imageData, selectedRegions, t) {
    ...
    const hopHeight = DEFAULT_HOP_HEIGHT_AMPLITUDE * Math.sin(phase * Math.PI * 2); // Hop height effect
    const forwardHop = DEFAULT_FORWARD_HOP_AMPLITUDE * Math.cos(phase * Math.PI * 2); // Forward hop effect
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and hop effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + Math.max(DEFAULT_HOP_HEIGHT_AMPLITUDE, DEFAULT_FORWARD_HOP_AMPLITUDE)));
        ...
        // Process expanded area for zoom and hop effects
...
self.onmessage = function(e) {
    try {
        const {
            ...
            hopHeightAmplitude = DEFAULT_HOP_HEIGHT_AMPLITUDE,
            forwardHopAmplitude = DEFAULT_FORWARD_HOP_AMPLITUDE
        } = e.data;
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            resultImageData = applyHopEffect(imageData, selectedRegions, value);
