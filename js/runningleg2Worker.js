...
// Modified poses for a skipping/hopping run style
const poses = [
    // Starting position - legs together, arms up
    { leftLeg: 0, rightLeg: 0, leftArm: -30, rightArm: -30 },
    // Push off - legs bent back, arms pushing down
    { leftLeg: 45, rightLeg: 45, leftArm: 30, rightArm: 30 },
    // Mid-air - legs spread, arms out
    { leftLeg: -15, rightLeg: 15, leftArm: -45, rightArm: 45 },
    // Landing - legs coming together, arms recovering
...
function applyRunningEffect(imageData, selectedRegions, value) {
    ...
    // Calculate vertical hop movement
    const hopHeight = 20;
    const verticalOffset = Math.abs(Math.sin(value * Math.PI * 2)) * hopHeight;
    ...
    const forwardOffset = Math.sin(value * Math.PI * 2) * 10;

    // Apply the running pose
    tempCtx.save();
    tempCtx.clearRect(0, 0, tempCanvas.width, tempCanvas.height);
    tempCtx.translate(
        tempCanvas.width / 2 + forwardOffset, 
        tempCanvas.height / 2 - verticalOffset
    );

    // Scale based on vertical position (smaller when higher)
    const scale = 1 - (verticalOffset / (hopHeight * 4));
    tempCtx.scale(scale, scale);

    // Draw body with slight tilt based on movement
    const tiltAngle = Math.sin(value * Math.PI * 2) * 5;
    tempCtx.rotate(tiltAngle * Math.PI / 180);
    ...
    // Draw legs with synchronized movement
    tempCtx.save();
    ...
    // Draw arms with exaggerated movement
    tempCtx.save();
    const armSwing = Math.sin(value * Math.PI * 2) * 15;
    tempCtx.rotate((currentPose.leftArm + armSwing) * Math.PI / 180);
    tempCtx.drawImage(canvas,
    ...
    tempCtx.save();
    tempCtx.rotate((currentPose.rightArm - armSwing) * Math.PI / 180);
    tempCtx.drawImage(canvas,
