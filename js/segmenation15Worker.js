self.onmessage = function(e) {
    const { imageData, value, selectedRegions, animationTime, debugMode } = e.data;
    
    try {
        if (!selectedRegions) {
            // Segmentation mode
            const result = segmentImage(imageData, value);
            self.postMessage({ segmentedImages: [result], isComplete: true });
        } else {
            // Animation mode
            const animatedResult = animateSegments(imageData, selectedRegions, animationTime);
            self.postMessage({ segmentedImages: [animatedResult], isComplete: true });
        }
    } catch (error) {
...
function animateSegments(imageData, selectedRegions, t) {
    ...
    const swayOffset = swayAmplitude * Math.sin(phase * Math.PI * 2);
    ...
    // Apply animation to selected regions
    selectedRegions.forEach(region => {
        region.forEach(pixelIndex => {
            ...
            // Apply effects
            nx += swayOffset;
            ny += 0.05 * Math.sin(phase * Math.PI * 2);
