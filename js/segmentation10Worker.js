...
// Find the nearest segmented color
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            if (!visited[index]) {
                ...
                if (segment.length >= 350) { // Reduced minArea from 100 to 50
...
self.onmessage = function (e) {
    const { imageData, threshold, currentIteration, progress } = e.data;

    try {
        // Perform segmentation
        const resultImageData = segmentImage(imageData, threshold);
