...
function moveRegionUp(imageData, totalMove) {
    ...
    for (let y = 0; y < height; y++) {
        const newY = Math.max(y - totalMove, 0); // Changed to subtract and use Math.max
        if (newY !== y) {
...
function moveSelectedRegions(imageData, selectedRegions, maxVerticalOffset) {
    ...
    selectedRegions.forEach(region => {
        ...
        region.forEach(pixelIndex => {
            ...
            const newY = Math.max(0, y - verticalOffset); // Changed to subtract and use Math.max
...
self.onmessage = function(e) {
    ...
    try {
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            ...
            moveRegionUp(resultImageData, totalMove); // Changed function name to moveRegionUp
            currentIteration++;
            progress = undefined;
        }
