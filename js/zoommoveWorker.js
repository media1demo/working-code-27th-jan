// Placeholder for zoommoveWorker.js
...
//     const cycleLength = 6; // Number of frames in the swing cycle
//     const phase = (t * cycleLength) % 1; // Current phase of the swing cycle
//     const swingAngle = 0.1 * Math.sin(phase * Math.PI * 2); // Swing angle
...
//             nx += swingAngle * Math.cos(phase * Math.PI * 2);
//             ny += swingAngle * Math.sin(phase * Math.PI * 2);
...
const DEFAULT_CYCLE_LENGTH = 8; // Number of frames in the dance cycle
const DEFAULT_ZOOM_AMPLITUDE = 0.2; // Amplitude of the zoom effect
const DEFAULT_MOVE1_AMPLITUDE = 0.1; // Amplitude of the first movement
const DEFAULT_MOVE2_AMPLITUDE = 0.05; // Amplitude of the second movement
...
function applyDanceEffect(imageData, selectedRegions, t) {
    const { width, height } = imageData;
    const newImageData = createTransparentImageData(width, height);

    // Animation parameters
    const cycleLength = DEFAULT_CYCLE_LENGTH;
    const phase = (t * cycleLength) % 1; // Current phase of the dance cycle
    ...
    const move1 = DEFAULT_MOVE1_AMPLITUDE * Math.sin(phase * Math.PI * 2); // First movement
    const move2 = DEFAULT_MOVE2_AMPLITUDE * Math.cos(phase * Math.PI * 4); // Second movement
    ...
    for (let regionIndex = 0; regionIndex < selectedRegions.length; regionIndex++) {
        ...
        // Calculate expanded bounds to allow for zoom and dance effects
        const padding = Math.ceil(Math.max(width, height) * (zoomAmplitude + Math.max(DEFAULT_MOVE1_AMPLITUDE, DEFAULT_MOVE2_AMPLITUDE)));
        ...
        // Process expanded area for zoom and dance effects
...
self.onmessage = function(e) {
    try {
        const {
            ...
            move1Amplitude = DEFAULT_MOVE1_AMPLITUDE,
            move2Amplitude = DEFAULT_MOVE2_AMPLITUDE
        } = e.data;
        ...
        if (selectedRegions?.length > 0 && selectedRegions[0]?.length > 0) {
            resultImageData = applyDanceEffect(imageData, selectedRegions, value);
